...
class ErrorHandlingScalar_throw_domain_error : public ::testing::Test {
 public:
  ...
  std::string expected_message_with_message(T y) {
    ...
    expected_message << "function: " << "y" << " error_message " << y
                     << " after y";
  ...
  void test_throw(T y) {
    try {
      stan::math::throw_domain_error<T>("function", "y", y, "error_message ", " after y");
      FAIL()
    ...
    try {
      stan::math::throw_domain_error<T>("function", "y", y, "error_message ");
      FAIL()
          << "expecting call to throw_domain_error<> to throw a domain_error, "
          << "but threw nothing";
    } catch (std::domain_error& e) {
